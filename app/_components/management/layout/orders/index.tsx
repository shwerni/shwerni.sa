...
import { isEnglish, findPayment, findUser } from "@/utils";
...
export default function EmployeeOrders({
  ...
  // data
  const [data, setData] = React.useState<OrdersTable[]>(iOrders);

  // pagmention states
  const [page, setPage] = React.useState(1);
  const [totalPages, setTotalPages] = React.useState(
    Math.ceil(iTotalCount / 10),
  );
  ...
  async function getOrders() {
    ...
    try {
      // get orders
      const data = await getOrdersForManagments(page, debounced);
          ...
          <Dialog>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                {/* copy order id */}
                <DropdownMenuItem
                  onClick={() => navigator.clipboard.writeText(String(zid))}
                >
                  {labels.copyZid}
                </DropdownMenuItem>
                {/* copy order id */}
                <DropdownMenuItem
                  onClick={() => navigator.clipboard.writeText(String(czid))}
                >
                  {labels.copyEnc}
                </DropdownMenuItem>
                {/* copy client phone */}
                ...
                {/* edit order */}
                <Link href={`${findUser(role)?.url}orders/${order.oid}`}>
                ...
                {order.payment?.payment === PaymentState.PAID &&
                  order.meeting.map((i, index) => (
                    <Link
                      key={index}
                      href={`${findUser(role)?.url}meetings/${order.oid}?session=${i.session}`}
                    >
                      <DropdownMenuItem>{labels.editMeetings}</DropdownMenuItem>
                    </Link>
                  ))}
              </DropdownMenuContent>
            </DropdownMenu>
