...
// New Component for the Red Block Line
...
// Ensure TypeScript knows messages can have a blocked flag
...
export default function ChatClient({
  ...
  const visibleMessages = (() => {
    ...
    if (data.blocked) {
      // find the exact timestamp of the latest block marker
    ...
    return data.messages.filter((msg: Messages) => {
      ...
      // hide after the red block marker line to the user
      if (msg.blocked && !isOwner) return false;

      // hide messages sent by the user the red block marker
      if (
        data.blocked &&
        blockTimeMs !== null &&
        isOwner &&
        ...
        // user's message was sent at same time or AFTER the block marker
  ...
  return (
    <div className="flex flex-col h-[70vh] sm:h-[calc(100vh-4rem)] sm:max-w-4xl sm:mx-auto bg-slate-50 sm:border-x border-slate-200 sm:shadow-sm font-sans">
      <header className="shrink-0 flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 bg-white border-b border-slate-200 z-10">
        <div className="flex items-center gap-3">
          <div className="relative w-12 h-12 rounded-full overflow-hidden border border-slate-100 shadow-sm shrink-0">
      ...
      {isChatExpired ? (
        <div className="shrink-0 px-3 sm:px-6 py-5 bg-slate-50 border-t border-slate-200 flex flex-col items-center justify-center gap-3 z-10 text-center">
          ...
          <p className="text-sm text-slate-500 max-w-sm">
            لقد مر 48 ساعة على موعد الجلسة وتم إغلاق المحادثة. يمكنك حجز جلسة
