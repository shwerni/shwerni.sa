    ...
    <SheetClose asChild>
      <Link
        ...
        className={cn(
          ...
          active
            ? "bg-theme-200/10 text-theme-200"
            : "text-gray-600 hover:bg-gray-100 hover:text-gray-900",
          className,
        )}
      >
        {active && (
          <span className="absolute right-0 top-1/2 -translate-y-1/2 -translate-x-1/2 w-1 h-[80%] bg-theme-200 rounded-r-full" />
        )}
        ...
        {active && (
          <span className="mr-auto w-1.5 h-1.5 rounded-full bg-theme-200" />
        )}
      </Link>
    </SheetClose>
  );
