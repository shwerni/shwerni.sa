...
export default function ScaleAssessment({ scaleSlug, scaleTitle, items, resultRanges }: Props) {
  const router = useRouter();

  // answers: { [itemId]: selectedOptionValue }
  ...
  const progressPct  = started ? Math.round(((currentIndex) / totalItems) * 100) : 0;
    ...
    return (
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div className="p-8 text-center">
          ...
          {/* Range badge */}
          {matchedRange && (
            <div className={`inline-flex items-center px-4 py-1.5 rounded-full text-sm font-medium border ${colors.text} ${colors.bg} ${colors.border} mb-5`}>
              {matchedRange.label}
              <span className="mx-1 text-gray-400">|</span>
              <span className="text-gray-500 font-normal">({matchedRange.minScore} - {matchedRange.maxScore})</span>
            </div>
          )}
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-xs text-gray-500 leading-6 text-right mb-6">
            ...
            هذا المقياس لأغراض التوعية فقط، وليس تشخيصاً طبياً. للتأكد من النتيجة،
            تفضل بمشاركة النتائج مع معالجك المختص. من فضلك لا تعتمد فقط على هذه النتيجة.
          </div>
  ...
  if (!started) {
    return (
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 text-center">
        ...
        <h2 className="text-lg font-semibold text-gray-900 mb-2">المقياس</h2>
        <p className="text-sm text-gray-500 mb-1">عدد الأسئلة: {totalItems} سؤال</p>
        <p className="text-xs text-gray-400 mb-6">لا توجد إجابة صحيحة وأخرى خاطئة — أجب بصدق على ما يتوافق مع مشاعرك</p>
        <button
    ...
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-5 text-xs text-gray-400">
          <span>{currentIndex + 1} من أصل {totalItems}</span>
